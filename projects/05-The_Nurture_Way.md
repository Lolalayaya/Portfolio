...
我們預想懷孕媽媽會經過三個階段使用媽媽餵遊戲化衛教 App，並分別以八角框架的核心驅動力設計對應功能，讓每個功能都能追溯回「希望媽媽做出什麼行為」。

**搜尋階段（Discovery）**：目的是讓媽媽接觸到媽媽餵資訊後，願意上網站瀏覽、互動並註冊試用。

預期行為：到網站瀏覽／讀媽媽百科／報名媽媽教室／看使用者見證 → 嘗試試用版 → 完成註冊。

| # | 設計解方 | 驅動力 |
| --- | --- | --- |
| 1 | 標語：「寶貝的健康不能等，妳願意為台灣寶貝的哺育健康盡一份心力嗎？」 | 使命與召喚 |
| 2 | 初階拖拉式知識小遊戲，測試哺乳相關小知識 | 發展與成就 |
| 3 | 首頁益智遊戲，通過後註冊即可領取獎勵 | 創意與賦權 |
| 4 | 標語：「好的哺乳衛教工具，是值得妳花時間投資的」 | 所有權與佔有 |
| 5 | 一年兩次折扣，提供給一次訂閱一年的懷孕媽媽 | 稀缺性與迫切感 |
| 6 | 展示已取得證書的資深媽媽 | 稀缺性與迫切感 |

**上手階段（Onboarding）**：目的是讓媽媽選擇 6 個成長路徑學習，並持續被誘導學習、了解自己與他人的進度，同時導入媽媽社群。

預期行為：學習增長 XP → 選擇 6 個成長路徑（懷孕篇／醫院篇／上班篇／外出篇／在家篇／飲食篇）→ 看影片並留言 → 加入社群並自我介紹 → 遵循高階會員建議。

| # | 設計解方 | 驅動力 |
| --- | --- | --- |
| 1 | 標語：「妳被選出來證明，媽媽餵遊戲化衛教 App 可以改變妳的哺育生活，過得更好」 | 使命與召喚 |
| 2 | 會員 PK（知識王形式），檢測衛教知識程度、增加 XP | 發展與成就 |
| 3 | 顯示各成長路徑的實力值增長 | 發展與成就 |
| 4 | 隨機推送三個每日任務影片 | 發展與成就 |
| 5 | 成長路徑雷達圖隨知識增長改變形狀 | 創意與賦權 |
| 6 | 挑戰哺乳相關知識以獲得獎勵 | 創意與賦權 |
| 7 | 客製化學習路徑，完成後可獲得認證證書 | 創意與賦權 |
| 8 | 顯示某位媽媽有幾位 FB 朋友也加入會員 | 社會影響力 |
| 9 | 打開每日限時問題箱子（每日小測驗）獲得 coins | 稀缺性與迫切感 |
| 10 | 打開彩蛋箱，獲得撫育幼兒小道具 | 未知性與好奇心 |
| 11 | 親餵教學工作坊，限時報名才有折扣 | 損失與避免 |

**支撐黏著階段（Scaffolding）**：目的是讓媽媽持續登入、收集金幣、撫育虛擬寶寶，並與成功哺乳的資深媽媽互動，形成長期黏著。

預期行為：每週登入兩次 → 瀏覽新內容 → 收集金幣（一天 1～2 次）→ 檢視 dashboard 與下一級的差距 → 與社群對談 → 撫育虛擬寶寶、購買虛擬商店商品。

| # | 設計解方 | 驅動力 |
| --- | --- | --- |
| 1 | 可選擇將部分金幣（%）轉換成現金，捐給哺乳公益團體 | 使命與召喚 |
| 2 | 撫育虛擬寶寶時，可客製化自己與寶寶的角色形象 | 創意與賦權 |
| 3 | 每完成一個單元（含多個問題集），開寶物箱獲得 coins 及撫育道具 | 所有權與佔有 |
| 4 | 依媽媽在某學習路徑上的偏好，推薦最相關的影片 | 所有權與佔有 |
| 5 | 在自己的 blog 撰寫使用經驗文章（如泌乳器使用文），可換取金幣 | 所有權與佔有 |
| 6 | 每日計算並顯示十大排行榜排名（近百種排行榜，如「本週飲食路徑觀看最多影片者」），刻意避免同幾位固定霸榜，讓每次看榜都有成就感 | 發展與成就 |
...
- 客製化形象頁面
![客製化形象頁面](../images/The_Nurture_Way/客製化形象頁面.png "客製化形象頁面")
撫育虛擬寶寶（支撐黏著階段-預期行為）
