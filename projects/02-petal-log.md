...
MVP 上線後，Should-have 清單中的心情紀錄以外項目（症狀紀錄、提醒通知的異常提醒版本、備註／匯出的 PDF 報表版本）皆已實作；為了不違背「首頁極簡」原則，這些進階功能一律採**預設收合／關閉（opt-in）**，只在設定裡手動開啟後才會出現在月曆或日詳情中。
...
| 背景 | `#FBF6F3` 暖白 | 降低臨床感 |
| 文字 | `#453936` 暖灰黑 | 比純黑柔和 |
| 點綴（易孕期／排卵日） | `#4F9D8C` 薄荷綠 | 與主色/輔色區隔，避免誤讀 |
| 選配階段色（濾泡期／黃體期） | `#C98A2B` 琥珀 ／ `#6F8FB0` 灰藍 | 僅在設定中個別開啟對應週期階段顯示時才出現在月曆上 |

字級系統以「一眼看懂天數」為核心：最大的 Display 字級（2.5rem）只留給「第幾天」「幾月幾號」這類使用者最常掃視的數字，其餘標題與內文則維持克制的層級，避免畫面被過多字重搶走焦點。四階段色條與症狀顏色皆不是寫死的樣式，而是存進使用者設定，可在設定面板用色票個別調整，兼顧預設的一致性與客製彈性。
...
Petal Log 提供一個以月曆為核心的生理週期紀錄網站，讓使用者能快速記錄、生理週期預測與查看歷史紀錄；在 MoSCoW 的 Must-have 全數完成後，已進一步將多數 Should-have 項目也做了出來，並持續以 SDD（軟體設計文件）追蹤每次迭代的變更。
...
已實作（Should-have）：
- 排卵日／易孕期預測（附醫療免責提示）
- 週期四階段（月經期／濾泡期／排卵期／黃體期）月曆色條標示，顏色可自訂
- 伴隨症狀記錄，含內建症狀顯示/隱藏、自訂症狀與自由文字備註
- 週期異常偵測（經期過長／週期不規律）與主畫面提醒
- 週期報表：摘要統計、週期歷史表、症狀頻率統計，可透過瀏覽器列印另存為 PDF

未來規劃：
- 情緒追蹤
- 提醒通知（Web Notification / PWA）
- 原始資料匯出（CSV / JSON）與匯入，作為雲端同步前的過渡方案
- 雲端同步／多裝置備份
- AI 洞察建議
- 週期圖表統計
...
目前以自動化瀏覽器測試（Playwright）走過完整流程——首次引導、快速記錄、月曆填色、預測計算、編輯/刪除、設定切換——確保每個功能實際可操作，而不只是看起來完成。過程中也實際抓出並修正了兩個真實的邏輯錯誤：一是手動輸入未來日期的紀錄時，「目前第幾天」會因為抓錯參考週期而消失；二是點選日期時，經量選項會因為元件沒有正確重新掛載而短暫顯示錯誤的預設值。

已另外撰寫一份完整的易用性測試流程文件，規劃 9 項任務（首次引導、快速記錄、看懂預測、補記錄/修改、找到症狀功能、自訂症狀與備註、週期階段顏色、異常提醒與報表匯出、自由探索），並針對「進階功能預設收合/關閉」這個設計選擇，特別設計了可發現性（discoverability）驗證任務，附上量化指標模板與 P0/P1/P2 問題分級標準；尚未實際招募使用者執行。

### Results

從設計文件到可公開瀏覽的 MVP 網站一次到位，部署在 [Petal Log](https://lolalayaya.github.io/Petal-Log/)，不僅涵蓋 MoSCoW 中全部的 Must-have 功能（生理期紀錄、月曆、預估日期、編輯紀錄），也進一步做出排卵/易孕期預測、週期四階段標示、症狀記錄、異常偵測與可列印報表等 Should-have 功能，並持續用 SDD 文件（現行版本 v1.10）記錄每次迭代的架構與設計決策。

### Future Improvements

- 依已撰寫完成的易用性測試流程，找主要／次要人物誌兩類真實使用者實際測試，優先驗證「症狀功能預設收合／關閉」是否犧牲了可發現性，以及「首次引導只出現一次」「自動填充後續天數」等假設是否真的降低使用負擔
- 累積更多真實紀錄後，回頭檢視預測演算法的準確度，並視情況調整平均週期/經期天數的計算方式，同時評估是否需要排除異常值
- 實作機器可讀的原始資料匯出/匯入（CSV / JSON），作為雲端同步前的過渡備份方案
- 依 MoSCoW 的 Should-have 剩餘項目與 Could-have 清單，接續評估心情紀錄、提醒通知、雲端同步等功能
